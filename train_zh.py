...
import sys
import math
from model import BDCN,visualizer,geometry
...
def cross_entropy_loss2d(loss_func,inputs,kps_src_list,kps_trg_list,effect_num_list,cuda=True):
    ...
    for i in range(b):
        ...
        for j in range(0,effect_num_list[i]):
            ...
            weights[j,:] = geometry.BilinearInterpolate([kps_src[0,j],kps_src[1,j]],inputs[i])  
            targets[j] = geometry.getBlurredGT([kps_trg[0,j],kps_trg[1,j]],featsShape)        
...
def cross_entropy_loss2d_onehot(loss_func,inputs, kps_src_list, kps_trg_list, effect_num_list, cuda=True):
    ...
    for i in range(b):
        ...
        for j in range(0,effect_num_list[i]):
            ...
            x,y = geometry.findNearestPoint([kps_src[0,j],kps_src[1,j]],featsShape)
            
            
            weights[j,:] = inputs[i,int(y),int(x),:].view(-1)
            
            x,y = geometry.findNearestPoint([kps_trg[0,j],kps_trg[1,j]],featsShape)
            index = int(y * w + x)
...
def train(model, args):
    ...
    trainDset = download.load_dataset(benchmark, datapath, thres, device, 'trn', '')
    trainDataloader = DataLoader(trainDset, batch_size=5, num_workers=0,shuffle=True,collate_fn=dataset.collate_func)
    valDset = download.load_dataset(benchmark, datapath, thres, device, 'val', '')
    valDataloader = DataLoader(valDset, batch_size=5, num_workers=0,collate_fn=dataset.collate_func)
    ...
    for key, v in params_dict.items():
        ...
        if re.match(r'conv[1-5]_[1-9]*_down', key):
            ...
            if 'weight' in key:
                params += [{'params': v, 'lr': base_lr, 'weight_decay': weight_decay, 'name': key}]
            elif 'bias' in key:
    ...
    for step in range(start_step, max_iter + 1):
        ...
        for i in range(args.iter_size):
            if cur == iter_per_epoch:
                ...
                epoch_loss = 0
                torch.save(model.state_dict(),'ckp_fcn_pfpascal/'+cur_time+'_'+str(int(step/iter_per_epoch))+'.pth.tar')
                for idx, data in enumerate(valDataloader):
                    ...
                    with torch.no_grad():
                        ...
                        for k in range(4):
                            ...
                            loss += cross_entropy_loss2d(loss_func,out[k][0],data['src_kps'],data['trg_kps'],data['effective_kps_num'])/curBatchSize
                            loss += cross_entropy_loss2d(loss_func,out[k][1],data['trg_kps'],data['src_kps'],data['effective_kps_num'])/curBatchSize
                ...
                if is_best:
                    torch.save(model.state_dict(),'ckp_fcn_pfpascal/'+cur_time+'_best')
            ...
            if cur % log_interval == 0:
                visualizer.visualizer_in_one(data,cur,out,save_path=args.visualizer)
                print('TRAIN Epoch: {} [{}/{} ({:.0f}%)]\t\tLoss: {:.6f}'.format(int(step/iter_per_epoch), cur ,iter_per_epoch, 100. * cur / len(trainDataloader),batch_loss))
            cur += 1                      
        # update parameter
        optimizer.step()
...
def parse_args():
    ...
    parser.add_argument('--visualizer', type = str, default = 'visualized_training_')
